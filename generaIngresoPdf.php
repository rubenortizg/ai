...
if (isset($_SESSION['usuario'])) {
  ...
  $pdf->SetTitle('Ingreso # ' . $ingreso['nrecibo'] . ' - ' . $ingreso['pnombre']. $ingreso['papellido']);
  ...
  $pdf->Cell(8);
  $pdf->Cell(21,9, 'Ingreso #', 0, 0, 'L');
  $pdf->SetFont('Arial','IU',14);
  $pdf->Cell(69,9, $ingreso['nrecibo'], 0, 0, 'L');
  $pdf->SetFont('Arial','',14);
  ...
  $pdf->Output('I', 'Comprobante # ' . $ingreso['nrecibo'] . ' - ' . $ingreso['pnombre'].' '.$ingreso['papellido']);
